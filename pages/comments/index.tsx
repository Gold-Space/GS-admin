/*
 * @FilePath: /GS-admin/pages/comments/index.tsx
 * @author: Wibus
 * @Date: 2022-02-12 14:11:01
 * @LastEditors: Wibus
 * @LastEditTime: 2022-02-13 17:06:13
 * Coding With IU
 */
...
const CommentsLists: NextPage = (anyProps: any) => {
...
CommentsLists.getInitialProps = async (ctx: any) => {
  const data = await $axios.get(`comments/list`).then( (res) => {
    return {
      id: res.data.map((item: { id: string; }) => item.id),
      name: res.data.map((item: { name: string; }) => item.name),
      description: res.data.map((item: { description: string; }) => item.description),
